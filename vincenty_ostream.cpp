 ...
 * value (from xalloc) is then assigned one of the enum values which represents
 * the type of manipulator just found in the stream.
 *
 * What happens later, how the manipulator is implemented and formats the
 * stream is up to the operator<<( std::ostream& os, const Position& rhs ). The
 * operator<< just fetches the xalloc value and looks at the enum value.
 */
